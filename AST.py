class Node(object):
    def acepta(self, visitor, table = None):
        return visitor.visit(self)
    def setParent(self, parent):
        self.parent = parent

class Programa(Node):
    def __init__(self, estatutos_dec_vars, declara_funciones, metodo_principal):
        self.estatutos_dec_vars = estatutos_dec_vars
        self.declara_funciones = declara_funciones
        self.metodo_principal = metodo_principal
        self.children = ( estatutos_dec_vars, declara_funciones, metodo_principal )
    def toString(self, i):
        return self.estatutos_dec_vars.toString(i) + \
               (self.declara_funciones.toString(i) if("toString" in dir(self.declara_funciones)) else '')+ \
               self.metodo_principal.toString(i)

class Declara_variables(Node):
    def __init__(self, tipo, lista_ids,declara_variables):
        self.tipo = tipo
        self.lista_ids = lista_ids
        self.declara_variables = declara_variables
        self.children = (lista_ids, declara_variables)
    def toString(self, i):
        return self.tipo.toString(i) + \
               self.lista_ids.toString(i) + \
               self.declara_variables.toString(i)

class Estatutos_dec_vars(Node):
    def __init__(self, tipo, lista_ids,estatutos_dec_vars):
        self.tipo = tipo
        self.lista_ids = lista_ids
        self.estatutos_dec_vars = estatutos_dec_vars
        self.children = (lista_ids, estatutos_dec_vars)
    def toString(self, i):
        return self.tipo.toString(i) + \
               self.lista_ids.toString(i) + \
               self.estatutos_dec_vars.toString(i)
